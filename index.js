...
const getAllUserFunc = async () => {
	...
	console.log("getUserFunc -> allUsers", allUsers);
	return allUsers;
};
app.get("/api/", (req, res) => {
	getAllUserFunc().then((a) => {
		res.send(a);
	});
});
app.post("/api/accessCode", (request, response) => {
	...
	// 	res.send(a);
	// });
	let url = `https://www.linkedin.com/oauth/v2/accessToken?grant_type=authorization_code&code=${request.body.code}&redirect_uri=https://one-fm-job-application-portal.vercel.app/form.html&client_id=78q6xv6hcsf430&client_secret=PVnMPHKawL5UBgRD`;
axios.post(url)
.then((res) => {  
